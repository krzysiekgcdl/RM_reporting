tab3_fun <- function(variables, all_data, my_doc, section_header_name) {
  ...
  data_tab3$subject_completed_entire_course <- data_tab3[[subject_completed_entire_course]]
  data_tab3$reason_for_premature_termination <- data_tab3[[reason_for_premature_termination]]
  
  value_included_status <- dt3_variables %>%
    filter(reason_for_premature_termination == "Yes") %>%
    pull(ValueIncluded)
  
  if (value_included_status == "No") {
    data_tab3$reason_for_premature_termination <- ifelse(
      data_tab3$reason_for_premature_termination == "Yes", "No",
      ifelse(data_tab3$reason_for_premature_termination == "No", "Yes",
             data_tab3$reason_for_premature_termination)
    )
  }
  
  original_cols_to_remove <- unique(c(subject_completed_entire_course, reason_for_premature_termination))
  data_tab3 <- data_tab3[, !(names(data_tab3) %in% original_cols_to_remove)]
  
  data_tab3 <- data_tab3 %>%
    select(
      PatientNr,
      Site,
      subject_completed_entire_course,
      reason_for_premature_termination,
      everything() # This will include all other columns in their current order
    )
  ...
  if (all(is.na(data_tab3$subject_completed_entire_course))) {
    ...
    # Ensure the column is a factor with "Yes" and "No" levels
    data_tab3$subject_completed_entire_course <- factor(
      data_tab3$subject_completed_entire_course,
      levels = c("Yes", "No")
    )

    data_tab3$reason_for_premature_termination <- factor(
      data_tab3$reason_for_premature_termination,
      levels = c("Yes", "No")
    )
    ...
    listing_3 <- fontsize(listing_3, i = NULL, j = ~., size = 8, part = "body")
    listing_3 <- fontsize(listing_3, i = NULL, j = ~., size = 9, part = "header")
    listing_3 <- theme_box(listing_3)
    # listing_3 <- width(listing_3, j = ~., width = c(1, 1, rep(1, 8)))
